...
class TorPiece extends GamePiece {
    ...
    public int[][] moveable(GamePiece[][] board) {
        ...
        // 4 + 7 is maximum possible move for tor piece (not include itself) so 5 col
        // will be 4 and 8 row will be 7
    ...
    public int[][] capturable(GamePiece[][] board) {
        ...
        // tor piece can move orthogonally only but can go any distance
        // check one by one starting from i = 1 to 7 the maximum
        // this is captureable step, so any direction detect have piece, it will store
        // the position if is another side piece and stop that direction
        // four direction have their own capturable move, maybe there is no piece in
        // that direction
        // the while loop condition is based on the four direction
        while (top || bottom || left || right) {
            // this is for moving top
