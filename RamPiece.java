...
class RamPiece extends GamePiece
{
    ...
    {
        // ram piece move forward or backward only, so using current image to identify it
        // for example, starting the blue side move with name blue the current image is 0, so is move forward
        // while red side moving, is turning the board and the current image will change
        // but in constructer i set red and blue flip as 1, so will become 0 after changing the image :)
        int r = row + (currentImage == 0? -1 : 1); 

        // check the r is in the board range
        if(r >= 0 && r < 8)
        {   
            // this is moveable, so only for empty space then return the position that can move 
            if(board[r][col] == null)
            {
    ...
    private void reachTheEndCheck()
    {
        ...
        // why i need updated, because ram piece will turning itself every turn if ram piece still at the end of the board
        // so updated is for make sure that ram piece turn around one time, even not moving ram piece
